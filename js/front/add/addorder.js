...
addorder.controller('addorderCtrl', ['$scope', 'APIService', function ($scope, APIService) {
    ...
    $scope.addOrder = function () {
        ...
        $scope.order.accidentLongitude = sessionStorage.getItem('accident_address_nar_lng')
        $scope.order.accidentLatitude = sessionStorage.getItem('accident_address_nar_lat')
        if (!isPhone.test($scope.order.accidentDriverPhone)) {
            ...
            APIService.add_order($scope.order).then(function (res) {
                if (res.data.http_status == 200) {
                    ...
                    if(res.data.http_code == 'company.car.size.no.exist'){
    ...
    $scope.$watch('order.carSize', function (accident) {
        if (accident == '' || accident == null) {
            $('#submit').addClass('button_disabled').attr("disabled", 'disabled');
            $scope.counts8 = 0;
        } else {
    ...
    //     }
    // });
    $scope.$watch('counts1 + counts2  + counts4 + counts5 + counts6 + counts7 + counts8', function (newValue, oldValue) {
        if (!$scope.tuixiu) {
            if (newValue == 6) {
                $('#submit').removeAttr("disabled").removeClass('button_disabled');
            } else {
    ...
    //     $scope.initData();
    // }
    $scope.chargeMode = function (type, num) {
        switch (type) {
            case 4:
                $scope.mode = num;
                sessionStorage.setItem('chargeMode', num);
                break;
            case 3:
                $scope.counts7 = 1;
                if (num == 1) {
            ...
            case 1:
                $scope.counts8 = 1;
                if (num == 1) {
                    $scope.order.carSize = 1;
                } else {
                    $scope.order.carSize = 2;
                }
                $scope.carSize = num;
                sessionStorage.setItem('addorder_order', JSON.stringify($scope.order));
                break;
    ...
    $scope.initData = function () {
        ...
        if (sessionStorage.getItem('addorder_order') != null) {
            ...
            if (JSON.parse(sessionStorage.getItem('addorder_order')).hasOwnProperty('carSize')) {
                $scope.carSize = JSON.parse(sessionStorage.getItem('addorder_order')).carSize;
            } else {
                $scope.carSize = '';
            }
        }
